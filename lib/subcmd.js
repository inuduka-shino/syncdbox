...
module.exports = (function () {
  ...
  function savePackages(packages) {
    ...
    return Promise.all(packages.map((pkg) => {
      ...
      return co(function *() {
        ...
        // TODO: ??? results = yield [prms, prms2, ...] ??
        const metaInfos = yield shelterPackage
          .save(targetPath, pkg.files).map((prms) => {
            prms.then((fileinfo) => {
              const absPath = fileinfo[0];
              const pw = fileinfo[1];
              return [path.relative(targetPath, absPath), pw];
            });
          });
  ...
  function loadPackages(packages) {
    ...
    return Promise.all(packages.map((pkg) => {
      ...
      return co(function *() {
        yield shelterPackage.syncPull();
        const metaInfo = yield shelterPackage.readMetaInfo();
        const metaInfoAbs = metaInfo.files.map((fileInfo) => {
          const file = fileInfo[0];
          const pw = fileInfo[1];
          return [path.resolve(loadTargetPath, file), pw];
        });

        yield makeDirWhenNoDir(loadTargetPath);
        yield shelterPackage.load(metaInfoAbs);
        return metaInfoAbs.map((fileInfo) => {
          return fileInfo[0];
        });
      }).then((absFiles) =>{
